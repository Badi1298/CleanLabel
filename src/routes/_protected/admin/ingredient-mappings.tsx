...
import {
	...
	unmappedIngredientsQueryOptions,
} from "#/queries/ingredient-queries";
import type { getUnmappedIngredients } from "#/server/ingredient-functions";
import { mapOffIngredientToIngredient } from "#/server/ingredient-functions";

export const Route = createFileRoute("/_protected/admin/ingredient-mappings")({
...
type UnmappedTagData = NonNullable<
	Awaited<ReturnType<typeof getUnmappedIngredients>>["data"]
>[0];
...
function MappingRowActions({ tag }: { tag: string }) {
	...
	const mapTagFn = useServerFn(mapOffIngredientToIngredient);
	const [selectedIngredientId, setSelectedIngredientId] = useState("");

	const { data: ingredientsResult } = useQuery({
		...ingredientsQueryOptions({
			pageIndex: 0,
			pageSize: 1000,
			globalFilter: "",
	...
	const handleMap = async () => {
		...
		try {
			const res = await mapTagFn({
				data: { tag, ingredientId: selectedIngredientId },
			});
	...
	return (
		<div className="flex items-center gap-2">
			<Select value={selectedIngredientId} onValueChange={setSelectedIngredientId}>
				...
				<SelectContent>
					{ingredientsResult?.data.map((ing) => (
			...
			<Button onClick={handleMap} disabled={!selectedIngredientId}>
				Map
			</Button>
		</div>
	);
...
function RouteComponent() {
	...
	const { data: result } = useQuery({
		...unmappedIngredientsQueryOptions(queryArgs),
